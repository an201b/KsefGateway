# ==========================================
# СКРИПТ: ПРЯМАЯ ОТПРАВКА XML (ИСПРАВЛЕННЫЙ)
# ==========================================

# 1. Формируем XML (Here-String)
$xmlContent = @"
<tns:Faktura xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:tns="http://crd.gov.pl/wzor/2023/06/29/12648/" xsi:schemaLocation="http://crd.gov.pl/wzor/2023/06/29/12648/ schemat.xsd">
 ...
 <tns:Podmiot2>
  <tns:DaneIdentyfikacyjne>
   ...
   <tns:Nazwa>DOZ SPÓŁKA AKCYJNA DIRECT SP.K.</tns:Nazwa>
  </tns:DaneIdentyfikacyjne>
  <tns:Adres>
   <tns:KodKraju>PL</tns:KodKraju>
 ...
 <tns:Fa>
  ...
  <tns:P_2>TEST-$(Get-Random)</tns:P_2>
...
# !!! ИСПРАВЛЕНИЕ: ПРИНУДИТЕЛЬНО ПРЕВРАЩАЕМ В ОДНУ СТРОКУ !!!
# Это гарантирует, что ConvertTo-Json не сделает массив
if ($xmlContent -is [array]) {
    $xmlContent = $xmlContent -join "`r`n"
}

# Отправляем в наш локальный DebugController
$url = "http://localhost:5062/api/debug/send-xml"
Write-Host "Sending XML to Local Gateway ($url)..." -ForegroundColor Cyan

# Создаем объект для отправки
$payload = @{
    xmlContent = $xmlContent
}
# Depth 10 нужен, чтобы PowerShell не обрезал глубокие объекты (хотя тут плоский объект)
...
try {
    ...
    Write-Host "   KSeF Ref: $($response.elementReferenceNumber)" -ForegroundColor Green
